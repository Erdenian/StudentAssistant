...
internal class HomeworkEditorScreen(private val arguments: HomeworkScreen.HomeworkEditor) : Screen {

    @Composable
    override fun Content() {
        val viewModel = viewModel {
            findComponent<HomeworksApi, HomeworksComponent>()
                .homeworkEditorViewModelFactory
                .getInternal(arguments.semesterId, arguments.homeworkId, arguments.subjectName)
        }
        ...
        LaunchedEffect(done) {
            if (done) {
                navigator.pop()
                lessonNameToCreate?.let { name ->
                    navigator.push(ScreenRegistry.get(ScheduleScreen.LessonEditor(viewModel.semesterId, name)))
                }
            }
        }
