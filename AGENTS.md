...
* **Плагин Kover:** При добавлении тестов в модуль необходимо обязательно подключать плагин `alias(libs.plugins.kover)`
  в `build.gradle.kts` этого модуля. Иначе модуль не будет участвовать в подсчете общего процента покрытия.
