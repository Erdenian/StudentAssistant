...
* **Внедрение зависимостей:** `Analytics` следует внедрять в **ViewModel**. Избегайте отправки событий напрямую из UI
  (Composable-функций), за исключением глобальных событий навигации.
* **Подключение:** Для использования аналитики в фиче необходимо добавить зависимость
  `implementation(project(":features:analytics:api"))` в `build.gradle.kts`.
...
* **Параметры:** `screen_class` (SimpleName класса маршрута, например, `ScheduleRoute`, `HomeworkEditorRoute`).
* **Правило:** Вручную отправлять событие `screen_view` внутри экранов **запрещено**, чтобы избежать дублирования.

#### 2. Пользовательские действия
Отправляйте события при успешном завершении значимых действий.
* **Создание/Редактирование:** `lesson_created`, `lesson_edited`, `homework_created`.
    * *Рекомендуемые параметры:* ID сущности не обязателен для аналитики поведения, важнее качественные характеристики
      (например, `subject_name`, `is_recurring` для урока).
