...
## 10. UI Previews
* **Обязательность:** Все Composable-компоненты и экраны должны иметь Preview функции.
* **Покрытие:** Preview должны покрывать все возможные визуальные состояния компонента.
* **Сценарии:**
    * **Темы:** Обязательно проверяйте как светлую, так и темную тему (используйте
      `@Preview(uiMode = Configuration.UI_MODE_NIGHT_YES)`).
    * **Данные:** Создавайте отдельные Preview для состояний загрузки (Loading), отсутствия данных (Empty) и наличия
      данных (Content).
    * **Граничные случаи:** Проверяйте отображение очень длинных текстов (например, используйте объекты `.long` из
      `SampleData`).
* **Обертка:** Все Preview должны быть обернуты в `AppTheme`, чтобы корректно подтягивались цвета, типографика и
  кастомные размеры.
