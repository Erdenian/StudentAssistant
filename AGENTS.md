...
* **Lesson**: В коде и комментариях переводить как **"Занятие"**, а не "Урок". Это универсальный термин для школы и
  вуза.
* **Semester**: Семестр или четверть. В коде и комментариях переводить как **"Расписание"**.
...
* **Assisted Injection:** Активно используется для создания ViewModel с параметрами (например, `id` занятия),
  передаваемыми через навигацию. Используйте `@AssistedInject` и `@AssistedFactory`.
* **Feature API Injection:** В клиентский код (например, ViewModels) всегда внедряйте интерфейс **Api** фичи (например,
  `RepositoryApi`, `AnalyticsApi`), а не отдельные репозитории или утилиты. Доступ к конкретным сущностям получайте
  через свойства этого Api.
...
* **Запрещено:** Использовать стиль Conventional Commits (не писать `feat:`, `fix:`, `test:`). Начинайте сообщение с
  большой буквы.
...
* **contentDescription:** Для всех интерактивных элементов (`IconButton`, кнопки с иконками, кликабельные иконки в полях
  ввода) **обязательно** указывать осмысленное описание через `contentDescription`.
* **Запрет null:** Использовать `contentDescription = null` разрешено **только** для декоративных элементов, которые не
  несут смысловой нагрузки и не меняют состояние приложения.
...
Для сбора статистики использования приложения используется модуль `:features:analytics`.

### 14.1. Основные принципы
* **Интерфейс:** Взаимодействие происходит через интерфейс `ru.erdenian.studentassistant.analytics.api.Analytics`.
* **Точка вызова:** Логика отправки событий должна находиться во **ViewModel**. Избегайте отправки событий напрямую из
  UI.
...
* **Регистр:** Всегда используйте **`snake_case`** (строчные буквы, разделение подчеркиванием).
* **События (Events):**
    * Шаблон: `object_action` или `context_action`.
    * Примеры: `lesson_created`, `homework_deleted`, `settings_changed`, `app_opened`.
* **Параметры (Params):**
    * Шаблон: `snake_case`.
    * Примеры: `lesson_type`, `weeks_count`, `is_done`.
* **Значения:** Если значение является перечислением (enum) или фиксированным набором, также используйте `snake_case`
  (например, `lecture`, `seminar`). Динамические пользовательские данные (названия предметов) передаются как есть.

### 14.3. Типы событий

#### 1. Просмотр экранов (`screen_view`)
* **Статус:** Реализовано **автоматически**.
* **Где:** В `StudentAssistantApp.kt` через отслеживание изменений `NavigationState`.
* **Параметры:** `screen_class` (SimpleName класса маршрута).
* **Правило:** Вручную отправлять событие `screen_view` внутри экранов **запрещено**.
* **Контекст:** Не добавляйте параметры типа `screen` или `source` в другие события вручную. Аналитика автоматически
  связывает события с последним `screen_view`.

#### 2. Воронки и намерения
Отправляйте события, обозначающие намерение пользователя, чтобы строить воронки конверсии.
* **Начало действия:** `homework_add_clicked`, `lesson_add_clicked`.
* **Выбор элемента:** `homework_clicked`, `lesson_clicked` (с параметрами, например, названием предмета).

#### 3. Результат действий
Отправляйте события при успешном завершении действий.
* **Создание/Редактирование:** `lesson_created`, `lesson_edited`, `homework_created`.
    * *Рекомендуемые параметры:* ID сущности не обязателен для аналитики поведения, важнее качественные характеристики
      (например, `subject_name`, `type`).
* **Удаление:** `lesson_deleted`, `homework_deleted`.
* **Настройки:** При изменении важных настроек (например, `default_start_time_changed`).
