...
* Используется кастомная система отступов через `LocalDimensions` (файл `Dimensions.kt` в `:core:style`). Избегайте
  хардкода `dp` в layout, используйте `MaterialTheme.dimensions`.
...
* **Flow:** Используйте `StateFlow` в ViewModel для UI-состояния.
