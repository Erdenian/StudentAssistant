...
class HomeworkEditorActivity : AppCompatActivity() {
    ...
    private val homework: Homework? by lazy {
        ScheduleManager.getHomeworkOrNull(
            semester.id,
            intent.getLongExtra(HOMEWORK_ID, -1L)
        )
    }
        ...
        when (item.itemId) {
            ...
            R.id.menu_lesson_editor_save -> {
                ...
                homework?.let { homework ->
                    ScheduleManager.updateHomework(
                        semester.id,
                        homework.copy(subjectName, description, deadline)
                    )
                } ?: run {
                    ScheduleManager.addHomework(
                        semester.id,
                        Homework(subjectName, description, deadline)
                    )
                }
