...
class LessonInformationActivity : AppCompatActivity(), OnScheduleUpdateListener {
  ...
  private val lessonId: Long by lazy {
    intent.getLongExtra(LESSON_ID, -1L).takeIf { it != -1L } ?: throw IllegalStateException("Не передан id пары")
  }
  ...
  override fun onOptionsItemSelected(item: MenuItem): Boolean {
    when (item.itemId) {
      ...
      R.id.menu_lesson_information_edit_button ->
        startActivity<LessonEditorActivity>(SEMESTER_ID to semesterId, LESSON_ID to lessonId)
